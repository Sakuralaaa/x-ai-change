...
<div class="card"><div class="row"><input id="key" type="password" placeholder="CPA 管理密钥"><button class="primary" id="scan">扫描认证文件</button><button class="danger" id="fix">修复选中项（未选则修复全部）</button><span id="progress" class="muted"></span></div><p id="message" class="muted"></p></div>
...
document.querySelector('#rows').innerHTML=(s.accounts||[]).map(function(a){let operation=a.last_operation==='fixed'?'已修复':(a.last_operation==='failed'?'修复失败':a.last_operation);return '<tr><td><input type="checkbox" data-id="'+esc(a.auth_index)+'" '+(a.needs_fix?'checked':'')+'></td><td>'+esc(a.name)+'</td><td>'+esc(a.email)+'</td><td><code>'+esc(a.base_url||'(缺失)')+'</code></td><td>'+(a.has_using_api?String(a.using_api):'(缺失)')+'</td><td class="'+(a.error||a.needs_fix?'bad':'ok')+'">'+esc(a.error||operation||(a.needs_fix?'需要修复':'正常'))+'</td></tr>'}).join('')}
