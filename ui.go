...
.ok{color:#65d39a}.bad{color:#ff8290}.muted{color:#96a0bb}.pill{padding:3px 8px;border-radius:999px;background:#27314d}code{color:#9fc0ff}@media(max-width:700px){table{display:block;overflow:auto}}
</style></head><body><div class="wrap">
<h1>xAI 403 认证修复</h1><p class="muted">把 xAI OAuth 认证文件调整为 <code>base_url=https://api.x.ai/v1</code> 且 <code>using_api=true</code>。</p>
<div class="card"><div class="row"><input id="key" type="password" placeholder="CPA Management Key"><button class="primary" id="scan">扫描</button><button class="danger" id="fix">修复选中（未选则修复全部）</button><span id="progress" class="muted"></span></div><p id="message" class="muted"></p></div>
<div class="card"><div class="row"><span>目标地址：<code id="target"></code></span><span>备份目录：<code id="backup">-</code></span></div>
<table><thead><tr><th></th><th>文件</th><th>邮箱</th><th>base_url</th><th>using_api</th><th>状态</th></tr></thead><tbody id="rows"></tbody></table></div>
</div><script>
...
document.querySelector('#rows').innerHTML=(s.accounts||[]).map(a=>`<tr><td><input type="checkbox" data-id="${esc(a.auth_index)}" ${a.needs_fix?'checked':''}></td><td>${esc(a.name)}</td><td>${esc(a.email)}</td><td><code>${esc(a.base_url||'(缺失)')}</code></td><td>${a.has_using_api?String(a.using_api):'(缺失)'}</td><td class="${a.error||a.needs_fix?'bad':'ok'}">${esc(a.error||a.last_operation||(a.needs_fix?'需要修复':'正常'))}</td></tr>`).join('');}
...
document.querySelector('#scan').onclick=async()=>{try{await call('/scan',{method:'POST'});refresh()}catch(e){document.querySelector('#message').textContent=e.message}};
document.querySelector('#fix').onclick=async()=>{let ids=[...document.querySelectorAll('[data-id]:checked')].map(x=>x.dataset.id).filter(Boolean);if(!confirm(ids.length?`确认修复选中的 ${ids.length} 个文件？`:'未选择文件，将修复全部异常文件，继续？'))return;try{await call('/fix',{method:'POST',body:JSON.stringify({auth_indexes:ids})});refresh()}catch(e){document.querySelector('#message').textContent=e.message}};
refresh();
</script></body></html>`)
